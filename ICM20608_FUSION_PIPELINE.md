...
- 不创建另一套 GUI，不用新窗口代替现有主窗口；仅允许为 ICM20608 原始数据新增独立弹窗（图 5、图 6），弹窗不建立第二条数据流。
...
### 11.4 修改现有图表，并新增 ICM 弹窗（图 5、图 6）
...
- 图5（弹窗）：ICM20608 三轴角速度随时间变化，单位 `°/s`，与主窗口共用同一数据流。
- 图6（弹窗）：ICM20608 三轴加速度随时间变化，单位 `m/s²`；可选叠加转换到 MPU 坐标系后的结果，便于对照。
- 弹窗实现要求：
  - 由左侧控制区按钮打开/关闭，可同时打开；
  - 使用与主窗口相同的刷新定时器和数据缓存，不新增远程读取进程；
  - 关闭主窗口时弹窗一并关闭；
...
- 现有 GUI 布局与核心交互保留。
- 主窗口四个图表与 ICM 弹窗（图 5、图 6）均使用真实双传感器数据更新。
