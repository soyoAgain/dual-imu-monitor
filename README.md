...
- 图 3（左中）：传感器在 X/Y/Z 坐标中的三维空间轨迹估计（融合加速度 + 四元数重力补偿 + ZUPT），`INVALID` 状态暂停积分；
- 图 4（右中）：传感器姿态可视化，包含固定世界轴、重力方向、随姿态旋转的机体三轴、线圈法向和线圈平面；
- 图 5（左下）：ICM20608 三轴角速度；
- 图 6（右下）：ICM20608 三轴加速度，原始模式叠加转换到 MPU 坐标系后的结果。

左侧为控制与配置区：SSH 主机、有线接口、显示时间窗口、刷新间隔、轨迹速度阻尼、滤波系数 α、姿态增益 kp、四元数重力补偿与 ZUPT 开关、「显示原始数据」开关、开始/停止/清空按钮、连接状态、融合状态标签、ICM 状态、标定信息、采样指标和运行日志。采样指标中同时显示纯陀螺积分姿态和重力校正姿态的 roll/pitch/yaw。
...
界面左侧可开关“四元数重力补偿”和“零速修正 (ZUPT)”、调整 α 和 kp，并实时显示两种姿态角与静止状态。由于 MPU6050 无磁力计、yaw 会漂移，且加速度计存在零偏与噪声，图 3 仍只用于短时间运动趋势观察，不能视为准确的绝对空间坐标；可靠轨迹仍需外部定位或已知约束。
