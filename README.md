...
Mac 的有线网卡使用系统自动分配的 IPv4 Link-Local 地址，开发板的 `eth0` 固定为：
...
步骤七和步骤八暂时跳过，先实现 MPU6050 数据的 Mac 端实时监视界面。GUI 主程序位于 `gui/app.py`，启动入口为项目根目录的 `run_gui.sh`，固定使用 `pytorch_env` 中的 PyQt5、NumPy 和 Matplotlib。

### 界面布局

右侧为 2×2 网格：

- 左上图 1：X/Y/Z 三轴加速度随时间变化，单位为 `m/s²`；
- 右上图 2：X/Y/Z 三轴角速度随时间变化，单位为 `°/s`；
- 左下图 3：传感器在 X/Y/Z 坐标中的三维空间轨迹估计（四元数重力补偿 + ZUPT）；
- 右下图 4：传感器姿态可视化，包含固定世界轴、重力方向、随姿态旋转的机体三轴、线圈法向和线圈平面。

左侧为控制与配置区：SSH 主机、显示时间窗口、刷新间隔、轨迹速度阻尼、滤波系数 α、姿态增益 kp、四元数重力补偿与 ZUPT 开关、开始/停止/清空按钮、连接状态、采样指标和运行日志。采样指标中同时显示纯陀螺积分姿态和重力校正姿态的 roll/pitch/yaw。

点击“开始”后，Mac 自动将现有 `tools/monitor_rpmsg_imu.py` 上传到开发板 `/tmp`，再通过 SSH 流式接收 `/dev/ttyRPMSG0` 数据。左侧控制台会显示上传、连接、RPMsg 和错误信息，并实时显示采样序号、频率、温度、I²C 错误和 RPMsg 发送错误。

GUI 在打开 RPMsg TTY 前会核对 `remoteproc0`：状态必须为 `running`，固件必须为 `m4_rpmsg.elf`。`m4_openamp_probe.elf` 只用于通信对照试验，不会采集或发送 MPU6050 数据，GUI 会拒绝对它发送 `start`，避免旧版 `rpmsg_tty` 驱动异常。
...
轨迹图不再使用固定基线，而是采用与 `tools/attitude_rpmsg_imu.py` 相同的姿态处理链：

1. 启动后用约 100 帧静止数据估计陀螺零偏和初始重力方向；
2. 加速度与角速度经一阶低通滤波（系数 α，默认 0.2）；
3. 用 Mahony 互补滤波（增益 kp，默认 1.5）由陀螺积分和重力方向校正得到四元数；
4. 用四元数把加速度旋转到参考坐标系并减去重力矢量，得到线性加速度；
5. 检测静止状态（重力补偿后的线性加速度很小且角速度很小）时执行零速修正（ZUPT），把速度归零；
6. 对线性加速度二次积分，并可叠加可配置的速度阻尼。
...
界面左侧可开关“四元数重力补偿”和“零速修正 (ZUPT)”、调整 α 和 kp，并实时显示两种姿态角与静止状态。由于 MPU6050 无磁力计、yaw 会漂移，且加速度计存在零偏与噪声，图 3 仍只用于短时间运动趋势观察，不能视为准确的绝对空间坐标；可靠轨迹仍需外部定位或已知约束。当前模块 Y 轴饱和时，图 3 与重力校正姿态不可信，GUI 会给出饱和警告。

### 实现结果

- 已创建 `gui` 目录、PyQt5 主程序和 `run_gui.sh` 启动脚本；
- 已实现三轴加速度、三轴角速度、三维空间轨迹和三维姿态四幅实时图（2×2 布局）；
- 已实现 SSH 自动部署、数据流解析、开始/停止、清空、日志和错误提示；
- 已提供 `--demo` 演示模式，可在开发板离线时验证 GUI。
- 已实现 RPMsg `start/stop` 流控与单读取器排他锁；关闭 GUI 会先让远端采集程序发送 `stop`，不会遗留持续发送或多个读取进程。
