...
- 正点原子 STM32MP157 开发板；
- 正点原子 MPU6050 传感器模块；
- 开发板板载 ICM20608，与外接 MPU6050 刚性固定、整体运动；
- macOS + VS Code。

## 当前实现与验证边界（2026-09-22）

| 项目 | 当前实现 |
|---|---|
| 采集链路 | M4 以 50 Hz 采集 MPU6050，经 RPMsg 传给 Linux；Linux 读取板载 ICM20608，通过单条 SSH 流传给 Mac |
| 图 1 / 图 2 | 跨传感器加速度融合对照 / MPU 角速度；图 1 的融合结果不输入轨迹估计 |
| 图 3 | ICM20608 完整六轴与其读取时间 → 15 维 ESKF → 零速与静止 IMU 观测 → 最近 10 秒历史位置平滑及自动缩放 |
| 图 4 | 独立 MPU 陀螺积分姿态，初始朝向来自静止标定，随后会漂移 |
| 网络 | Mac `en7=169.254.50.1/16`，开发板 `169.254.50.2/16`；GUI 连接退出后自动重试，停止/关闭可取消 |
| 已验证 | 真实 50 Hz GUI 工作流、历史平滑一致性、物理网线拔插重连；9 项软件回归通过 |
| 尚未解决 | MPU6050 Y 轴异常；Bug 3 的物理过冲/位移精度与 Bug 4 的较大转动漂移仍待外部参考验证 |

图 3 的位置对应 **板载 ICM20608 测点**，不是外接 MPU6050 或线圈中心。坐标方向变换不等于测点位置变换。短时测试正常或曲线平滑，都不能证明真实位移准确。

启动前确认 `ssh mp157` 可连接，并按下文 GUI 启动说明选择自动部署 M4 或使用已运行固件。运行 `./run_gui.sh` 后点击“开始”，保持静止约 2 秒完成初始化。GUI 默认使用 `pytorch_env`，可用 `PYTHON_BIN` 指定其他具备依赖的解释器。

文档入口：[GUI 使用说明](gui/README.md)、[ESKF 参数与限制](gui/ESKF.md)、[网络诊断](NETWORK_DIAGNOSIS.md)、[双 IMU 实施记录](ICM20608_FUSION_PIPELINE.md)。下文步骤一至八及 Bug 排查经过保留阶段历史，其中“当前”指该实验当时；使用现行软件时以本节和步骤九为准。
...
以下是当时 SSH 配置完成后的整板重启结果，不代表最近一次固定 IP 修改后又完成了整板重启验收。
...
以下为早期目标架构，包含尚未实现的中断采样、200～500 Hz、M4 姿态解算及线圈测量融合。现行实现是 M4 50 Hz 轮询采集、Linux 双 IMU 传输、Mac GUI 解算，不应将下列规划视为已完成能力。
...
该阶段测试结束时固件保持 `running`，便于直接观察板载 `user-led`。这是受控验证：Linux 的 LED trigger 已设为 `none`，但 GPIO LED 驱动仍声明占用 PF3。正式项目中必须在设备树中禁用该 Linux LED 节点或把 PF3 明确分配给 M4，避免 A7 与 M4 同时操作同一 GPIO。
...
最终确认：M4 已按 20 ms 固定周期持续采集 MPU6050；连续序号和毫秒时间戳同步增长；测试期间没有周期超期或 I²C 错误。该阶段结束时开发板保持 `m4_mpu6050.elf` 运行，UART5 保持解绑，可直接进入步骤五的 RPMsg 数据传输实现。
...
步骤七和步骤八暂时跳过，先实现双传感器数据的 Mac 端实时监视界面。GUI 主程序位于 `gui/app.py`，启动入口为项目根目录的 `run_gui.sh`，默认使用 `pytorch_env` 中的 PyQt5、NumPy 和 Matplotlib，可通过 `PYTHON_BIN` 覆盖解释器。

### 界面布局

右侧为 3×2 网格：

- 图 1（左上）：跨传感器融合加速度对照，单位为 `m/s²`；开启「显示原始数据」后叠加 MPU Y 与 ICM 转换到 MPU 坐标系的 Y。该融合结果不输入图 3；
- 图 2（右上）：MPU 三轴角速度，单位为 `°/s`；原始模式叠加转换到 MPU 坐标方向的 ICM 数据，此处不代表完成两路采样时差校正；
- 图 3（左中）：板载 ICM20608 测点的三维轨迹，使用同一颗 IMU 的完整六轴进行 ESKF 估计，最近 10 秒历史位置平滑及等比例自动缩放；ICM 轨迹输入不满足有效性检查时暂停传播；
- 图 4（右中）：独立 MPU 陀螺积分姿态，包含世界轴、参考重力方向、机体三轴及假设安装关系下的线圈法向和平面；这不是图 3 的 ESKF 姿态，且会漂移；
...
GUI 在打开数据流前会核对 `remoteproc0`：状态必须为 `running`，固件必须为 `m4_rpmsg.elf`（代码兼容 `m4_rpmsg_fix1.elf`），且 `/dev/icm20608` 必须存在（否则提示先执行 `sh tools/install_icm20608.sh`）。RPMsg 读取还需要 `/dev/ttyRPMSG0`。`m4_openamp_probe.elf` 只用于通信对照试验，GUI 会拒绝使用。

### 启动方法

确保 `ssh mp157` 可以正常连接、ICM 驱动已加载。若关闭自动启动选项，需先确保 M4 采样固件运行且 `/dev/ttyRPMSG0` 存在；若使用默认自动启动选项，需准备本地已构建的 `m4/build-arm/m4_rpmsg.elf`，由安装脚本部署。然后在 Mac 项目目录执行：
...
GUI 默认使用 Mac 有线接口 `en7`，SSH/SCP 绑定此接口；每轮连接建立一个 SSH ControlMaster，固件检查、SCP 上传与数据流复用该连接。默认勾选“自动启动 M4，并启用 Linux 开机启动”：每次建立连接（包括自动重连）都会执行安装脚本，上传固件和服务、启用服务，并在固件未运行或不匹配时启动它，并非只在第一次执行。如需取消开机启动，在 Mac 项目目录执行：

```bash
sh tools/uninstall_m4_autostart.sh mp157
```

2026-09-21 曾发现板端固件和服务文件均为 0 字节，原因未查明；当时手动恢复了固件和本次采集，未恢复服务文件。后续是否成功安装/启用服务应以板端检查和安装日志为准，不能用“当前能采集”证明断电后会自启动。见 [网络恢复记录](NETWORK_DIAGNOSIS.md)。

### 断线与自动重连

Mac 有线使用前述固定 IP。GUI SSH 连接超时 3 秒，保活间隔 2 秒、最多 3 次未回应；连接退出后等待 2 秒重新连接，停止和关闭窗口均取消重试。配置错误也会重复尝试，持续失败时应停止并查看日志。

每次重连尝试清空旧数据和 ESKF，恢复后需静止约 2 秒重新初始化；不能补回断线期间未收到的运动数据。很短的断线可能由原 TCP 会话直接恢复，无需创建新会话。已收到数据中的 ICM 时间间隔异常另由轨迹时间检查处理。

实测主动断开 SSH 会话后约 3.1 秒恢复，物理网线链路恢复后约 7.8 秒恢复 50 Hz 数据。后者包含 macOS 地址重新就绪及 SSH 重建，测试关闭了自动安装 M4 选项；这些是单次结果，不是固定时延保证。见 [完整验收记录](diagnostic_logs/network_reconnect_20260921/README.md)。
...
2. 使用同一颗 ICM20608 的完整三轴加速度和角速度（转为 rad/s），加速度扣标定偏置，两者以同一旋转矩阵转到 MPU 坐标方向；用 ICM 读取时间计算积分间隔，估计的位置仍对应 ICM 测点；
3. 传播位置/速度/局部姿态误差/加速度偏置/陀螺偏置的 15×15 协方差；
4. 按最近 20 个有效样本（约 0.4 秒）的原始 IMU 稳定性判断静止；勾选 ZUPT 时每帧更新零速度，每 5 个确认静止帧另更新重力比力和陀螺偏置观测，采用 Joseph 协方差更新及姿态误差重置；
5. ICM 有效标志、标定、数据形状/有限性、时间差绝对值 ≤30 ms、非零且未超量程加速度和角速度均须通过检查；失败时跳过传播、清除静止窗口并断开积分时间连续性。恢复的首个有效样本仅建立时间基准，不补积分；这类跳帧不会自动重置全部滤波状态。存在上一个有效时间基准时，时间倒退或间隔超过 0.1 秒才清空重标定；
6. 保存最近 10 秒历史位置与当前误差状态的交叉协方差，使观测同时修正历史点和当前状态，避免两者不一致产生回折；三轴等比例自动缩放，清空同步重置滤波器和协方差。

实现约定、噪声默认值和适用限制见 [ESKF 说明](gui/ESKF.md)。

图 4 使用独立的**纯陀螺积分**四元数（初始朝向来自静止标定，之后只积分扣除零偏的角速度），初始化后不再用加速度持续校正这个四元数，可观察 roll/pitch/yaw 的相对变化；但它没有重力校正，roll/pitch 会随零偏和积分误差持续漂移，yaw 本来就无绝对参考，界面已标注“会漂移”。

### 双传感器融合

图 1 的对照曲线逐帧运行 `tools/fusion_state.py` 的状态机，把 ICM20608 加速度经标定转换到 MPU 坐标方向后参与融合；这些状态不等同于图 3 的轨迹输入有效性：

- `MPU`：ICM 不可用时只用 MPU；
- `融合`：两路都可信时按权重融合 Y 轴；
- `ICM 补偿`：MPU Y 轴饱和（硬饱和 ≥19.612 m/s² 立即降级，带迟滞退出）时用 ICM 转换值替代；
- `无效`：MPU Y 不可用且 ICM 也不可用时，不输出融合加速度；若 MPU Y 可用，即使 ICM 不可用仍可进入 `MPU` 状态，但图 3 不会回退到 MPU 或拼轴输入。
...
- 已实现 SSH 自动部署、断线重试、单流双传感器解析、开始/停止、清空、日志和错误提示；
- 已提供 `--demo` 演示模式，用于离线布局和内部诊断；不能替代实机验收或证明运动精度；
...
以下保留定位过程，不代表当前结论；已确认根因和修复见本条开头及下方“根因与修复”。
...
**当前验收状态与限制：** 首轮修改时因保留用户采集而暂未实机验证；后续已完成真实 GUI 开始/停止/清空/重启、历史平滑及进程清理验收，详见本条“最新进展”。物理过冲和绝对位移精度仍未验证。纯 IMU 无法可靠区分匀速运动与静止，也不能保证回到物理原点时估计位置归零，本条 Bug 保持开放。
...
- 报告问题时，旧版轨迹使用 MPU6050 的 X/Z 加速度、板载 ICM20608 的 Y 加速度（或加权 Y）及 MPU6050 角速度。原流水线要求两者刚性固定；用户随后确认两者刚性固定、整体移动，因此已排除本次“相对运动导致标定失效”的假设。刚性固定不意味着两个测点在旋转时具有相同的线加速度，跨测点拼轴仍需处理位置差及时间差。
- 阶段四曾测得两路约 33 ms 的固定时差；当前采集器将 M4 时间映射到接收时间并随后读取 ICM，没有实现这项固定时差对应的历史样本对齐。GUI 只显示标定中的时差信息，当时未用于跨 IMU 的 ESKF 输入对齐。新版 ICM 完整六轴轨迹已不依赖该拼接。这是旧版旋转时需验证的误差来源，尚未确认其对此次数十米漂移的贡献。
...
早期规划的目标链路如下；当前已完成 RPMsg/SSH/GUI 链路，线圈测量接入与位移精度仍不在已验证范围内：
